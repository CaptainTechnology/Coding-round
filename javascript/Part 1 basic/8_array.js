...
// // Create arrays
// let arr = [10, 20, 30];
// let arr2 = [1, 2, 3, { name: "Captain" }];
// let emptyArr = new Array(5); // length 5, empty slots
...
// // Reverse
// arr.reverse();      // reverses original array

// // Sparse array
// arr[100] = 99;      // empty slots created, length = 101

// Notes:
// - Arrays can store numbers, strings, objects, functions.
// - Only use Array(5) if you really want empty slots.
// - In strict mode, octal literals like [01] are not allowed.
...
// 🔹 Definition:
// Cloning an array means creating a **copy of an array**.  
// There are two types of copies:
// 1. Shallow Copy → copies top-level elements only, nested objects/arrays still shared.
// 2. Deep Copy → completely independent copy, nested objects/arrays also copied.
...
// // 1️⃣ Using slice()
// // slice() returns a shallow copy. Can take start & end indexes. Negative indexes allowed.
// let arr1 = [1, 2, 3, 4, { city: "Delhi" }];
// let copySlice = arr1.slice();        // copy all
// let copySlicePartial = arr1.slice(0, 3); // copy first 3 elements
// let copySliceNeg = arr1.slice(0, -2);    // copy except last 2 elements

// // 2️⃣ Using spread operator
// let copySpread = [...arr1]; // shallow copy

// // 3️⃣ Using Array.from()
// let copyFrom = Array.from(arr1); // shallow copy
// // Array.from() can also take a map function:
// const arrLike = [10, 20, 30];
// let mappedArr = Array.from(arrLike, (x, i) => x + i); // [10, 21, 32]

// // 4️⃣ Using concat()
// let arr2 = [4, 5, 6];
// let combined = arr1.concat(arr2, 100, 200); // shallow copy + combine arrays
// // Note: nested objects/arrays are shared
// combined[4].city = "Mumbai";  
// console.log(arr1[4].city); // Mumbai → original affected
...
// // 1️⃣ Using JSON.parse(JSON.stringify(array))
// // Creates a deep copy but has limitations (cannot copy Date, Map, Set, circular refs)
// let deepArr = JSON.parse(JSON.stringify(arr1));
// deepArr[4].city = "Siddharth Nagar";
// console.log(arr1[4].city);       // Delhi → original unaffected

// // 2️⃣ Using structuredClone()
// // Modern method that can handle complex objects including Date, Map, Set, RegExp, BigInt, circular references
// let structClone = structuredClone(arr1);
// structClone[4].city = "Kanpur";
// console.log(arr1[4].city);       // Delhi → original unaffected
...
// /*
// Shallow Copy:
// - Copies only top-level elements
// - Nested objects/arrays shared by reference
// - Methods: slice(), [...arr], Array.from(), concat()

// Deep Copy:
// - Completely independent copy
// - Nested objects/arrays copied
// - Methods: JSON.parse(JSON.stringify(...)) → simple objects only
//           structuredClone() → robust, works with complex types

// Use shallow copy for simple arrays, deep copy for nested/complex arrays.
// */


// // ===========================
// // 4. QUICK SYNTAX REFERENCE
// // ===========================

// /*
// slice():          arr.slice(), arr.slice(0, 3), arr.slice(0, -2)
// spread:           [...arr]
// Array.from():     Array.from(arr, mapFn?)
// concat():         arr.concat([a,b], x, y)
// JSON Deep Copy:   JSON.parse(JSON.stringify(arr))
// Structured Clone: structuredClone(arr)
// */
...
// // 4️⃣ Stack processing with do-while
...
// // =================== For-of Loop ===================
// // Used for iterables (arrays, strings, sets, maps). Gives direct value.

// const str = "hello captain";
...
// // Nested destructuring inside for-of loop
// const people = [["captain", 23], ["sodi", 14]];
// for (const [name, age] of people) {
//   console.log(name, age);
// }

// // =================== For-in Loop ===================
// // Used for object keys or array indexes
...
// // =================== Array Destructuring ===================
// // Extract values from arrays easily

// const arr = ["first", "second", "third", "fourth", undefined];
// const [v1, v2, v3, v4, v5 = "fifth"] = arr;
// console.log(v1, v2, v3, v4, v5); // first second third fourth fifth
...
// forEach
// iterate over array and return provided function for each element, and return undefined; it takes a callback function otherwise show error undefined is not a function
...
// ---------------------- forEach ----------------------
// 👉 Iterates over array elements, calls the callback for each item.
// 👉 Always returns undefined (does NOT return a new array).
// 👉 Syntax: arr.forEach((elem, index, arr) => {}, thisArg);
// 👉 thisArg: value to use as 'this' inside callback.

let arr = [1, 2, 3, 4, 5];

// ❌ Error if no callback:
// arr.forEach(); // TypeError: undefined is not a function

// ✅ Example with normal callback (this → window/global by default)
// arr.forEach(function (elem, index, arrs) {
//   console.log(elem, index, arrs);
//   // console.log(this); // window/global
// });

// ✅ Passing custom object as thisArg
// const obj = { name: "captain" };
// arr.forEach(function (elem) {
//   console.log(elem, this.name);
// }, obj); // this → obj

//-----------------------------------------------------
const data = [1, 2, 3];
const role = "GlobalRole"; // global variable

// ❌ Normal function → 'this' = global (window / undefined in strict mode)
data.forEach(function () {
  console.log("Normal:", this.role);
});

// ✅ Arrow function → 'this' comes from parent scope
data.forEach(() => {
  console.log("Arrow:", this.role); // uses outer scope variable directly
});

// ✅ Normal function + thisArg → force 'this'
data.forEach(function () {
  console.log("With thisArg:", this.role);
}, { role: "CustomRole" });

//-----------------------------------------------------

// 📌 Summary:
// - By default, 'this' inside forEach callback → window/global (in browsers).
// - Arrow fn inherits 'this' from parent scope (better inside objects).
// - You can pass a 2nd arg (thisArg) to explicitly set 'this'.
